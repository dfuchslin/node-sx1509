...
class SX1509 {
  ...
  async readByte(registerAddress) {
    const i2c = await i2cbus.openPromisified(this.busNumber)
    var data = await i2c.readByte(this.deviceAddress, registerAddress)
    await i2c.close()
    return data
  }
  ...
  async readWord(registerAddress) {
    const i2c = await i2cbus.openPromisified(this.busNumber)
    var data = await i2c.readWord(this.deviceAddress, registerAddress)
    await i2c.close()
    return data
  }
  ...
  async writeByte(registerAddress, writeValue) {
    ...
    const i2c = await i2cbus.openPromisified(this.busNumber)
    await i2c.writeByte(this.deviceAddress, registerAddress, writeValue)
    await i2c.close()
  }
  ...
  async writeWord(registerAddress, writeValue) {
    ...
    const i2c = await i2cbus.openPromisified(this.busNumber)
    await i2c.writeWord(this.deviceAddress, registerAddress, writeValue)
    await i2c.close()
  }
  ...
  async ledDriverInit(pin, freq = 1, log = false) {
    var tempWord
    var tempByte
