...
class SX1509 {
  ...
  async init() {
    ...
    var testRegisters = 0
    testRegisters = await this.readWord(SX1509.REG_INTERRUPT_MASK_A) // This should return 0xFF00
    // Then read a byte that should be 0x00
    if (testRegisters === 0xFF00) {
      // Set the clock to a default of 2MHz using internal
      await this.clock(SX1509.INTERNAL_CLOCK_2MHZ)
      return true
    }
    return false
  }
  ...
  async reset(hardware) {
    ...
    if (hardware) {
      ...
      var regMisc = await this.readByte(SX1509.REG_MISC)
      if (regMisc & (1 << 2)) {
        regMisc &= ~(1 << 2)
        await this.writeByte(SX1509.REG_MISC, regMisc)
      }
      // Reset the SX1509, the pin is active low
      await this.pinMode(this.pinReset, SX1509.OUTPUT) // set reset pin as output
      await this.digitalWrite(this.pinReset, SX1509.LOW) // pull reset pin low
      await this.delay(1) // Wait for the pin to settle
      await this.digitalWrite(this.pinReset, SX1509.HIGH) // pull reset pin back high
    } else {
      log('software reset')
      // Software reset command sequence:
      await this.writeByte(SX1509.REG_RESET, 0x12)
      await this.writeByte(SX1509.REG_RESET, 0x34)
    }
  }
  ...
  async pinMode(pin, inOut) {
    ...
    var modeBit
    if ((inOut === SX1509.OUTPUT) || (inOut === SX1509.ANALOG_OUTPUT)) {
      modeBit = 0
    } else {
      modeBit = 1
    }

    var tempRegDir = await this.readWord(SX1509.REG_DIR_B)
    if (modeBit) {
      tempRegDir |= (1 << pin)
    } else {
      tempRegDir &= ~(1 << pin)
    ...
    await this.writeWord(SX1509.REG_DIR_B, tempRegDir)

    // If INPUT_PULLUP was called, set up the pullup too:
    if (inOut === SX1509.INPUT_PULLUP) {
      await this.digitalWrite(pin, SX1509.HIGH)
    }

    if (inOut === SX1509.ANALOG_OUTPUT) {
  ...
  async digitalWrite(pin, highLow) {
    ...
    if ((0xFFFF ^ tempRegDir) & (1 << pin)) { // If the pin is an output, write high/low
      var tempRegData = await this.readWord(SX1509.REG_DATA_B)
      if (highLow) {
        tempRegData |= (1 << pin)
      } else {
        tempRegData &= ~(1 << pin)
      }
      await this.writeWord(SX1509.REG_DATA_B, tempRegData)
    } else { // Otherwise the pin is an input, pull-up/down
      var tempPullUp = await this.readWord(SX1509.REG_PULL_UP_B)
      var tempPullDown = await this.readWord(SX1509.REG_PULL_DOWN_B)

      if (highLow) { // if HIGH, do pull-up, disable pull-down
        tempPullUp |= (1 << pin)
        tempPullDown &= ~(1 << pin)
        await this.writeWord(SX1509.REG_PULL_UP_B, tempPullUp)
        await this.writeWord(SX1509.REG_PULL_DOWN_B, tempPullDown)
  ...
  async ledDriverInit(pin, freq = 1, logarithmic = false) {
    ...
    tempWord = await this.readWord(SX1509.REG_INPUT_DISABLE_B)
    tempWord |= (1 << pin)
    await this.writeWord(SX1509.REG_INPUT_DISABLE_B, tempWord)

    // Disable pull-up
    // Writing a 0 to the pin bit will disable that pull-up resistor
    tempWord = await this.readWord(SX1509.REG_PULL_UP_B)
    tempWord &= ~(1 << pin)
    await this.writeWord(SX1509.REG_PULL_UP_B, tempWord)

    // Set direction to output (REG_DIR_B)
    tempWord = (await this.readWord(SX1509.REG_DIR_B))
    tempWord &= ~(1 << pin) // 0=output
    await this.writeWord(SX1509.REG_DIR_B, tempWord)

    // Enable oscillator (REG_CLOCK)
    tempByte = await this.readByte(SX1509.REG_CLOCK)
    tempByte |= (1 << 6) // Internal 2MHz oscillator part 1 (set bit 6)
    tempByte &= ~(1 << 5) // Internal 2MHz oscillator part 2 (clear bit 5)
    await this.writeByte(SX1509.REG_CLOCK, tempByte)

    // Configure LED driver clock and mode (REG_MISC)
    tempByte = await this.readByte(SX1509.REG_MISC)
    if (logarithmic) {
    ...
    await this.writeByte(SX1509.REG_MISC, tempByte)

    // Enable LED driver operation (REG_LED_DRIVER_ENABLE)
    tempWord = await this.readWord(SX1509.REG_LED_DRIVER_ENABLE_B)
    tempWord |= (1 << pin)
    await this.writeWord(SX1509.REG_LED_DRIVER_ENABLE_B, tempWord)

    // Set REG_DATA bit low ~ LED driver started
    tempWord = await this.readWord(SX1509.REG_DATA_B)
    tempWord &= ~(1 << pin)
    await this.writeWord(SX1509.REG_DATA_B, tempWord)
  }
  ...
  async analogWrite(pin, iOn) {
    ...
    // Linear mode: Ion = iOn
    // Log mode: Ion = f(iOn)
    await this.writeByte(SX1509.REG_I_ON[pin], iOn)
  }
  ...
  async breathe(pin, tOn, tOff, rise, fall, onInt = 255, offInt = 0, log = SX1509.LINEAR) {
    // TODO
  ...
  async clock(oscSource = 2, oscDivider = 1, oscPinFunction = 0, oscFreqOut = 0) {
    ...
    await this.writeByte(SX1509.REG_CLOCK, regClock)
...
Object.keys(Constants).map(key => {
  Object.defineProperty(SX1509, key, {
    value: Constants[key],
    writable: false,
    configurable: false,
    enumerable: true
