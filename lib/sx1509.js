...
class SX1509 {
  ...
  init() {
    ...
    var testRegisters = 0
    testRegisters = this.readWord(Constants.REG_INTERRUPT_MASK_A) // This should return 0xFF00

    // Then read a byte that should be 0x00
    if (testRegisters === 0xFF00) {
      // Set the clock to a default of 2MHz using internal
      this.clock(Constants.INTERNAL_CLOCK_2MHZ)
      return true
    }
  ...
  readByte(registerAddress) {
    var readValue
    var timeout = Constants.RECEIVE_TIMEOUT_VALUE
  ...
  readWord(registerAddress) {
    var readValue
    var msb, lsb
    var timeout = Constants.RECEIVE_TIMEOUT_VALUE * 2
  ...
  reset(hardware) {
    ...
    if (hardware) {
      ...
      var regMisc = this.readByte(Constants.REG_MISC)
      if (regMisc & (1 << 2)) {
        regMisc &= ~(1 << 2)
        this.writeByte(Constants.REG_MISC, regMisc)
      }
      // Reset the SX1509, the pin is active low
      this.pinMode(this.pinReset, Constants.OUTPUT) // set reset pin as output
      this.digitalWrite(this.pinReset, Constants.LOW) // pull reset pin low
      this.delay(1) // Wait for the pin to settle
      this.digitalWrite(this.pinReset, Constants.HIGH) // pull reset pin back high
    } else {
      // Software reset command sequence:
      this.writeByte(Constants.REG_RESET, 0x12)
      this.writeByte(Constants.REG_RESET, 0x34)
    }
  }
  ...
  pinMode(pin, inOut) {
    ...
    var tempRegDir = this.readWord(Constants.REG_DIR_B)
    if (modeBit) {
      tempRegDir |= (1 << pin)
    } else {
      tempRegDir &= ~(1 << pin)
    ...
    this.writeWord(Constants.REG_DIR_B, tempRegDir)

    // If INPUT_PULLUP was called, set up the pullup too:
    if (inOut === Constants.INPUT_PULLUP) {
      this.digitalWrite(pin, Constants.HIGH)
    }
  ...
  digitalWrite(pin, highLow) {
    ...
    if ((0xFFFF ^ tempRegDir) & (1 << pin)) { // If the pin is an output, write high/low
      var tempRegData = this.readWord(Constants.REG_DATA_B)
      if (highLow) {
        tempRegData |= (1 << pin)
      } else {
        tempRegData &= ~(1 << pin)
      }
      this.writeWord(Constants.REG_DATA_B, tempRegData)
    } else { // Otherwise the pin is an input, pull-up/down
      var tempPullUp = this.readWord(Constants.REG_PULL_UP_B)
      var tempPullDown = this.readWord(Constants.REG_PULL_DOWN_B)

      if (highLow) { // if HIGH, do pull-up, disable pull-down
        tempPullUp |= (1 << pin)
        tempPullDown &= ~(1 << pin)
        this.writeWord(Constants.REG_PULL_UP_B, tempPullUp)
        this.writeWord(Constants.REG_PULL_DOWN_B, tempPullDown)
  ...
  ledDriverInit(pin, freq = 1, log = false) {
    ...
    tempWord = this.readWord(Constants.REG_INPUT_DISABLE_B)
    tempWord |= (1 << pin)
    this.writeWord(Constants.REG_INPUT_DISABLE_B, tempWord)

    // Disable pull-up
    // Writing a 0 to the pin bit will disable that pull-up resistor
    tempWord = this.readWord(Constants.REG_PULL_UP_B)
    tempWord &= ~(1 << pin)
    this.writeWord(Constants.REG_PULL_UP_B, tempWord)

    // Set direction to output (REG_DIR_B)
    tempWord = this.readWord(Constants.REG_DIR_B)
    tempWord &= ~(1 << pin) // 0=output
    this.writeWord(Constants.REG_DIR_B, tempWord)

    // Enable oscillator (REG_CLOCK)
    tempByte = this.readByte(Constants.REG_CLOCK)
    tempByte |= (1 << 6) // Internal 2MHz oscillator part 1 (set bit 6)
    tempByte &= ~(1 << 5) // Internal 2MHz oscillator part 2 (clear bit 5)
    this.writeByte(Constants.REG_CLOCK, tempByte)

    // Configure LED driver clock and mode (REG_MISC)
    tempByte = this.readByte(Constants.REG_MISC)
    if (log) {
    ...
    this.writeByte(Constants.REG_MISC, tempByte)

    // Enable LED driver operation (REG_LED_DRIVER_ENABLE)
    tempWord = this.readWord(Constants.REG_LED_DRIVER_ENABLE_B)
    tempWord |= (1 << pin)
    this.writeWord(Constants.REG_LED_DRIVER_ENABLE_B, tempWord)

    // Set REG_DATA bit low ~ LED driver started
    tempWord = this.readWord(Constants.REG_DATA_B)
    tempWord &= ~(1 << pin)
    this.writeWord(Constants.REG_DATA_B, tempWord)
  }
  ...
  analogWrite(pin, iOn) {
    ...
    // Linear mode: Ion = iOn
    // Log mode: Ion = f(iOn)
    this.writeByte(Constants.REG_I_ON[pin], iOn)
  }
  ...
  breathe(pin, tOn, tOff, rise, fall, onInt = 255, offInt = 0, log = Constants.LINEAR) {
  ...
  clock(oscSource = 2, oscDivider = 1, oscPinFunction = 0, oscFreqOut = 0) {
    ...
    this.writeByte(Constants.REG_CLOCK, regClock)
