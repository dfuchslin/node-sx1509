...
class SX1509 {
  ...
  async readByte(registerAddress) {
    ...
    console.log(`  readByte: deviceAddress:${this.deviceAddress} registerAddress:${registerAddress} readValue:${readValue}`)
    return readValue
  }
  ...
  async readWord(registerAddress) {
    ...
    console.log(`  readWord: deviceAddress:${this.deviceAddress} registerAddress:${registerAddress} msb:${data.buffer[0]}->${msb} lsb:${data.buffer[1]}->${lsb} readvalue:${readValue}`)
    return readValue
  }
  ...
  async writeByte(registerAddress, writeValue) {
    console.log(`  writeByte: deviceAddress:${this.deviceAddress} registerAddress:${registerAddress} writeValue:${writeValue}`)
    await this.openBus()
    await this.i2c.i2cWrite(this.deviceAddress, 1, Buffer.from([writeValue]))
  }

  async writeWord(registerAddress, writeValue) {
    console.log(`  writeWord: deviceAddress:${this.deviceAddress} registerAddress:${registerAddress} writeValue:${writeValue}`)
    await this.openBus()
    const msb = (writeValue & 0xFF00) >> 8
    const lsb = writeValue & 0x00FF
    await this.i2c.i2cWrite(this.deviceAddress, 2, Buffer.from([msb, lsb]))
  }
  ...
  async pinMode(pin, inOut) {
    console.log(`pinMode pin:${pin} inOut:${inOut}`)
    // from pinDir
  ...
  async digitalWrite(pin, highLow) {
    console.log(`digitalWrite pin:${pin} highLow:${highLow}`)
    // from writePin
  ...
  async ledDriverInit(pin, freq = 1, logarithmic = false) {
    console.log(`ledDriverInit pin:${pin} freq:${freq} logarithmic:${logarithmic}`)
    var tempWord
    var tempByte
  ...
  async analogWrite(pin, iOn) {
    console.log(`analogWrite pin:${pin} iOn:${iOn}`)
    // from pwm
